...
export const data = new SlashCommandBuilder()
  .setName(fr.slashCommands.addHP)
  .setDescription(fr.commandDescriptions.addHP)
  ...
  .addIntegerOption((option) =>
    option
      .setName("amount")
      .setDescription(fr.optionDescriptions.amount)
      .setRequired(true)
  );
...
export async function executeMessage(msg: Message) {
  if (msg.channel.isSendable()) {
    ...
    if (args.length != 3) {
      await msg.channel.send(fr.usage.addHP);
      return;
    }
